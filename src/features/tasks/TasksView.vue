    ...
    <template v-else-if="store.isAdmin">
      <div v-if="adminTasks?.length" class="flex flex-col gap-2">
        <RouterLink
          ...
          class="flex items-center gap-4 px-4 py-3 rounded-lg bg-card hover:shadow-md transition-shadow"
        >
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <p class="font-medium text-sm truncate">
              {{ (assignment.task as any)?.title }}
            </p>
            <p class="text-xs text-muted-foreground">
              {{ (assignment.task as any)?.event?.name }} · Límite
              {{
                dayjs((assignment.task as any)?.deadline).format("D MMM HH:mm")
              }}
            </p>
          </div>
