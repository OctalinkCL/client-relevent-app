...
watch(() => event.value?.is_public, (val) => {
...
<template>
  <div class="p-6 max-w-2xl mx-auto">
    ...
    <template v-else-if="event">
      ...
      <Card class="mb-4">
        <CardHeader>
          <CardTitle class="text-base">Flyer</CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <p v-if="flyerError" class="text-sm text-destructive mt-2">{{ flyerError }}</p>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle class="text-base">Visibilidad</CardTitle>
        </CardHeader>
        <CardContent>
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm font-medium">{{ isPublic ? 'Público' : 'Privado' }}</p>
              <p class="text-xs text-muted-foreground">
                {{ isPublic ? 'Visible para todos' : 'Solo visible para tu equipo' }}
              </p>
            </div>
            <Switch :model-value="isPublic" @update:model-value="onPublicToggle" />
          </div>
        </CardContent>
      </Card>
    </template>
